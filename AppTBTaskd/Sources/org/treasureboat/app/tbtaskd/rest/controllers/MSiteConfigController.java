...
public class MSiteConfigController extends JavaMonitorController {
	...
	private void pushValues(TBMonitor_SiteConfig newSiteConfig) {
		...
		newSiteConfig.values().removeObjectForKey(TBMonitor_Constants.PASSWORD);
		...
		newValues.removeObjectForKey(TBMonitor_Constants.PASSWORD);
		...
		if (!TBFString.stringIsNullOrEmpty(newHashedPassword)) {
			siteConfig().values().takeValueForKey(newHashedPassword, TBMonitor_Constants.PASSWORD);
		} else if (!TBFString.stringIsNullOrEmpty(currentHashedPassword)) {
			siteConfig().values().takeValueForKey(currentHashedPassword, TBMonitor_Constants.PASSWORD);
		}
